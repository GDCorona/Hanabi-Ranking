...
export default function Comments({ pageName }) {
    ...
    // NEW: Track how many comments are currently visible (starts at 10)
    ...
    // NEW: Helper function to load 10 more comments
    ...
    return (
        <div id="comment-section" className="w-[90%] max-w-5xl mx-auto mt-32 mb-20 p-10 rounded-3xl bg-(--buttonColor)/80 border border-(--shadowColor)/40 shadow-2xl">
            <div className="mb-20">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <textarea rows="4" placeholder="Write your comment..." required value={formData.text} onChange={e => setFormData({...formData, text: e.target.value})}
                        className="w-full resize-y p-4 border border-(--shadowColor)/30 rounded-xl bg-(--bgColor) text-(--textColor) placeholder-(--textColor)/50 focus:outline-none focus:ring-2 focus:ring-(--shadowColor) shadow-inner font-medium"></textarea>
                    
                    <div className="flex justify-between items-center pt-2">
                        {/* NEW: Display the total comment count here */}
                        <div className="text-(--textColor) font-bold text-lg opacity-80">
            ...
            <div className="space-y-8">
                {/* NEW: Slice the array based on visibleCount */}
            ...
            {visibleCount < comments.length && (
                <div className="flex justify-center mt-12">
                    <button 
                        ...
                        className="px-6 py-2 border-2 border-(--shadowColor) text-(--textColor) font-bold rounded-full transition-all duration-300 hover:bg-(--shadowColor) hover:text-(--bgColor) shadow-[0_0_10px_transparent] hover:shadow-[0_0_15px_var(--shadowColor)] cursor-pointer"
                    >
                        View More Comments
                    </button>
                </div>
