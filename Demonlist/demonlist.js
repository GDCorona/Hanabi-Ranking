...
import { demonList } from './demondata.js';

const listContainer = document.getElementById("demonList");
demonList.forEach((demon, index) => {
  const li = document.createElement("li");
  li.className = "hoverable";
  const p = document.createElement("p");
  p.textContent = demon.name;
  const img = document.createElement("img");
  img.src = demon.image;
  img.alt = demon.name;
  li.appendChild(p);
  li.appendChild(img);
  li.addEventListener("click", () => openModal(demon, index + 1));
...
function openModal(demon, rank) {
  modalTitle.textContent = `#${rank}-${demon.name}`;
  modalBody.innerHTML = ""; // Clear previous content

  const fields = [
    { label: "Difficulty", value: demon.difficulty },
    { label: "FPS", value: demon.fps },
    { label: "Attempts", value: demon.attempts },
    { label: "Worst fail", value: demon.worstFail },
    { label: "Enjoyment", value: demon.enjoyment },
    { label: "Completion Date", value: demon.date }
  ];
  ...
  a.href = demon.video;
  a.target = "_blank";
  a.textContent = demon.video;
  videoP.appendChild(a);
  modalBody.appendChild(videoP);
